...
func TestCodexClientModelsResponsePreservesGpt6Templates(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{slug: "gpt-6-astra", name: "GPT-6 Astra", context: 1050000, maxContext: 1050000, efforts: []string{"low", "medium", "high", "xhigh", "max", "ultra"}},
		{slug: "gpt-6-sol", name: "GPT-6 Sol", context: 272000, maxContext: 872000, efforts: []string{"low", "medium", "high", "xhigh", "max", "ultra"}},
		{slug: "gpt-6-luna", name: "GPT-6 Luna", context: 272000, maxContext: 872000, efforts: []string{"low", "medium", "high", "xhigh", "max"}},
	} {
		...
		if got := intFromAny(model["context_window"]); got != tc.context {
			t.Fatalf("%s context_window = %d, want %d", tc.slug, got, tc.context)
		}
		if got := intFromAny(model["max_context_window"]); got != tc.maxContext {
			t.Fatalf("%s max_context_window = %d, want %d", tc.slug, got, tc.maxContext)
		}
...
func TestUsagePluginForcedTierPreservesResponseTier(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			tracker := newRequestUsageTracker()
			plugin := &usagePlugin{tracker: tracker, defaultServiceTier: "priority"}
			ctx := internallogging.WithRequestID(context.Background(), "req-tier-fallback")
			plugin.HandleUsage(ctx, tc.record)
			payload, ok := tracker.finalize("req-tier-fallback", usageFinalizeInput{status: http.StatusOK})
			if !ok || payload.ServiceTier != tc.wantRequested || payload.ResponseServiceTier != tc.wantReported {
				t.Fatalf("request and response tiers were not preserved: %#v", payload)
			}
		})
	}
}

...
func TestMergeLocallyPinnedCodexClientModelsKeepsPinnedModelsWhenRemoteLags(t *testing.T) {
	...
	for _, pinnedSlug := range locallyPinnedCodexClientModelSlugs {
		...
		if pinnedSlug == codexBuiltinGPT6AstraModelID {
			wantContext = 1050000
		}
		if got := modelBySlug[pinnedSlug]["context_window"]; got != wantContext {
			t.Fatalf("%s context_window = %#v, want %v", pinnedSlug, got, wantContext)
		}
	}
...
func TestMergeLocallyPinnedCodexClientModelsPrefersRemotePinnedMetadata(t *testing.T) {
	...
	if got := modelBySlug[codexBuiltinGPT6LunaModelID]["context_window"]; got != float64(272000) {

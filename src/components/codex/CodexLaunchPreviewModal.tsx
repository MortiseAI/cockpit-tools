...
export interface CodexLaunchPreviewLaunchOptions {
  apiServiceLaunchMode?: CodexLocalAccessLaunchMode;
...
export function CodexLaunchPreviewModal({
  ...
  const configReady = serverOnlyMode || (loadedTarget === previewTargetKey && !loading &&
  ...
  }, [previewTargetKey, configReload, applyLoadedConfig, instanceId, mode, serverOnlyMode, setError]);
  ...
  const applyContextConfig = useCallback(async () => {
    if (serverOnlyMode || configBusy || contextConfigSaving || !loadedConfig) return;
    const contextWindow = Number.parseInt(contextWindowInput, 10);
    const compactLimit = compactLimitInput.trim()
      ? Number.parseInt(compactLimitInput, 10)
      : undefined;

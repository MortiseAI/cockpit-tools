...
pub async fn codex_local_access_activate(
    ...
    // An explicit server-only request must never launch or take over a selected
    // managed instance. Omitted mode keeps the existing managed-instance API.
    if !launch_target.is_default && launch_mode == Some(CodexLocalAccessLaunchMode::ServerOnly) {
    ...
    // 先停止目标 profile 的客户端，再写入 API Service 凭据。
    ...
    if user_config.codex_launch_on_switch {
        ...
        let launch_error = match crate::commands::codex_instance::codex_start_instance_with_prepared_profile(
            app.clone(),
